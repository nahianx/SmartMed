...
export default function DoctorFinderPage() {
  ...
  const { data: doctors = [], isLoading } = useDoctorSearch(debouncedQuery, specialization)

  const handleSavePreset = () => {
    if (!query && !specialization) return
    const next: Preset[] = [...presets, { name: query || specialization || "Saved search", query, specialization }]
    setPresets(next)
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {specsLoading ? null : specializations.map((spec: any) => (
                  <option key={spec.id} value={spec.name}>{spec.name}</option>
                ))}
              </select>
            </div>
            <Button variant="outline" size="sm" onClick={handleSavePreset} className="gap-2">
          ...
          {isLoading ? (
            ...
            <Card className="p-10 text-center">
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-slate-500 text-sm">Try another specialty or search term.</p>
              </div>
            </Card>
          ) : (
            <div className="grid md:grid-cols-2 gap-4">
              {filtered.map((doctor: Doctor) => (
                <Card key={doctor.id} className="p-4 space-y-2">
                  ...
                  <div className="flex items-center gap-2 text-sm text-slate-600">
                    ...
                    <span>{(doctor as any).clinic?.address || (doctor as any).clinic?.name || 'Clinic info unavailable'}</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
